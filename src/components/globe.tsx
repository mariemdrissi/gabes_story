...
function GlobeMesh({ animProgress }: { animProgress: number }) {
  ...
  useFrame((_, delta) => {
    ...
    if (animProgress <= 0) {
      ...
      mesh.rotation.y += (destY - currentY) * lerpSpeed
      mesh.rotation.x += (destX - currentX) * lerpSpeed
    }
  })
...
function Atmosphere() {
  return (
    <mesh scale={[2.05, 2.05, 2.05]}>
      <sphereGeometry args={[2, 64, 64]} />
      <meshBasicMaterial color="#12082e" transparent opacity={0.1} side={THREE.BackSide} />
    </mesh>
  )
}
...
function Stars() {
  ...
  return (
    <points geometry={geometry}>
      <pointsMaterial color="#ffffff" size={0.04} sizeAttenuation transparent opacity={0.5} />
    </points>
  )
}
...
function SceneContent({ animProgress, zoomLevel }: { animProgress: number; zoomLevel: number }) {
  return (
    <>
      <ambientLight intensity={0.35} />
      <directionalLight position={[5, 3, 5]} intensity={0.85} />
      <directionalLight position={[-3, -1, -3]} intensity={0.1} color="#6666ff" />
      <GlobeMesh animProgress={animProgress} />
      <Atmosphere />
...
export default function GlobeScene() {
  ...
  useEffect(() => {
    // After 2s, start rotating to Gabès
    const t1 = setTimeout(() => {
      let progress = 0
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      >
        <SceneContent animProgress={animProgress} zoomLevel={zoomLevel} />
      </Canvas>

      {/* Gabès label - appears after zoom */}
      {zoomLevel > 0.5 && (
        <div style={{
          position: 'absolute',
          bottom: '28%',
          ...
          zIndex: 10,
          pointerEvents: 'none',
          opacity: Math.min(1, (zoomLevel - 0.5) * 2),
          ...
          <div style={{
            ...
            fontWeight: 700,
            textShadow: '0 0 12px rgba(239,68,68,0.6), 0 0 30px rgba(239,68,68,0.2)',
